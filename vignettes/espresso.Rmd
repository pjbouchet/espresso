...
Model convergence and mixing are commonly ascertained both (1) graphically, by inspecting trace plots and checking that chains are clearly stable and well-mixed, and that posterior density distributions are smooth, and (2) numerically, using diagnostics such as those available in package `coda` [@Plummer2006].

Trace and density plots can be obtained using the `plot` method. Convergence is evidenced by chains that level off to a stable, stationary state. Good mixing is apparent in plots that show random scatter around a mean value, somewhat resembling a hairy [caterpillar](http://sbfnk.github.io/mfiidd/external_ref/hairy_caterpillar.jpg).
...
> **Note:** Autocorrelation plots an be obtained with `autocorr = TRUE`.

```{r echo=TRUE, message=FALSE, warning=FALSE}
# Plot for the range covariate only
plot(rj.posterior, param.name = "range", autocorr = TRUE)
```
...
> **Note:** When model selection is enabled (`model.select = TRUE`), dose-response curves are derived irrespective of model IDs, and therefore reflect model uncertainty. If dose-response functions are needed for a specific model of interest, then the `by.model` argument should be set to `TRUE`.
